...
void litepcie_dma_process(struct litepcie_dma_ctrl *dma)
{
    ...
    /* set / get dma */
    if (dma->use_writer)
        litepcie_dma_writer(dma->fds.fd, dma->writer_enable, &dma->writer_hw_count, &dma->writer_sw_count, &dma->writer_dropped_count);
    if (dma->use_reader)
        litepcie_dma_reader(dma->fds.fd, dma->reader_enable, &dma->reader_hw_count, &dma->reader_sw_count, &dma->reader_dropped_count);
    ...
    if (dma->fds.revents & POLLIN) {
        if (dma->zero_copy) {
            ...
            len = read(dma->fds.fd, dma->buf_rd, DMA_WR_BUFFER_TOTAL_SIZE);
            if (len < 0) {
                perror("read");
                abort();
            }
